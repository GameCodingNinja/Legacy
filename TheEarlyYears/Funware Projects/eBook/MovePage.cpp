...
int DisplayMovePageDlg( HINSTANCE hInst, HWND hwnd, int MovePage, int NumPages )
{
    int Result = MovePage;
    ...
    if( pMovePageDlg )
        Result = pMovePageDlg->CreateDialogWnd( hInst, hwnd, MovePage, NumPages );
...
int TMovePageDlg::CreateDialogWnd( HINSTANCE hInst, HWND hwnd, int MovePage, int NumPages )
{
...
BOOL TMovePageDlg::OnInitDialog( HWND hwnd, HWND hwndFocus, LPARAM lParam )
{
    ...
    {
        if( i == PageToBeMoved )
            strcpy( StrTmp, "Current Page To Be Moved" );
        else
            sprintf( StrTmp, "Move To Page No. %u", i+1 );
