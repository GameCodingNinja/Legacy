...
BOOL TConfigDlg::OnInitDialog( HWND hwnd, HWND hwndFocus, LPARAM lParam )
{
    ...
    {        
        ...
        {
            int renderTime = GetRenderTime( ResCheck[i], hInst );
        ...
        int ScreenResID = GetScreenResSizeID( GetScreenWidth(), GetScreenHeight() );
    ...
    // Setup the Render Speed
    ScrollBar_SetRange( GetDlgItem( hwnd, SCR_SPEED ), 0, MAX_FRAME_SKIP-1, TRUE );
    ScrollBar_SetPos( GetDlgItem( hwnd, SCR_SPEED ), Settings.GetRenderSpeed(), TRUE );
    SetScrollBarLabels( hwnd, GetDlgItem( hwnd, SCR_SPEED ), Settings.GetRenderSpeed() );

    // Set the size to screen button
    Button_SetCheck( GetDlgItem( hwnd, CHK_FILL_SCREEN ), Settings.GetSizeToFit() );

	// Set the random camera
    Button_SetCheck( GetDlgItem( hwnd, CHK_RANDOM_CAMERA ), Settings.GetRandomCamera() );

	// Set the camera rotation
    Button_SetCheck( GetDlgItem( hwnd, CHK_RANDOM_ROT ), Settings.GetRandomRot() );
...
void TConfigDlg::SetScrollBarLabels( HWND hwnd, HWND hScrollBar, int ScrollPos )
{
    ...
    else if( hScrollBar == GetDlgItem( hwnd, SCR_SPEED ) )
    {
        ...
        {
            case NO_SKIP_FRAMES: 
                Static_SetText( hControl, "Slower but smoothest" );
            break;
            case SKIP_1_FRAME: 
                Static_SetText( hControl, "Faster but not as smooth" );
            break;
            case SKIP_2_FRAMES: 
                Static_SetText( hControl, "Even Faster but less smooth" );
            break;
            case SKIP_3_FRAMES: 
                Static_SetText( hControl, "Fastest but a lot less smooth" );
            break;
        }
    }
}   // SetScrollBarLabels
...
void TConfigDlg::RecordUserSettings( HWND hwnd )
{
    ...
    // Save the render speed
    Settings.SetRenderSpeed( ScrollBar_GetPos( GetDlgItem( hwnd, SCR_SPEED ) ) );
	...
	// Save random camera
    Settings.SetRandomCamera( Button_GetCheck( GetDlgItem( hwnd, CHK_RANDOM_CAMERA ) ) );

	// Save camera rotation
    Settings.SetRandomRot( Button_GetCheck( GetDlgItem( hwnd, CHK_RANDOM_ROT ) ) );
...
/************************************************************************
*    FUNCTION NAME:         SetScreenResSizeID                                                             
*
*    DESCRIPTION:           Set the ID of the current screen res
*
*    FUNCTION PARAMETERS:
*    Input:    HWND hwndCtl     - Handle of the scroll bar control
*              int ScrollPos    - scroll bar position
...
DWORD GetRenderTime( int ResID, HINSTANCE hInst )
{
    ...
    testPoly.SetMaxLightDistance( 500000 );
    ...
    // Set it for a solid render
    for( i = 0; i < testPoly.GetTotalPolyCount(); ++i )
    {
        testPoly.GetPoly( i )->SetSolidColor( true );
    }
