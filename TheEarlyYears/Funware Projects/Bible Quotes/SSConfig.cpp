...
int BlitToScreen[MAX_RES_CHECK] = { 0, 0, 0, 0, 4, 5, 6, 7, 8 };
...
BOOL TConfigDlg::OnInitDialog( HWND hwnd, HWND hwndFocus, LPARAM lParam )
{
    ...
    SendMessage( GetDlgItem( hwnd, BTN_INFO_COLOR ), BM_SETIMAGE, (WPARAM)IMAGE_BITMAP, 
                 (LPARAM) (HANDLE)LoadBitmap( hInst, "colors" ) );

    // Disable the other resolution controls
    if( Settings.GetUseScreenRes() )
    {
        Button_Enable( GetDlgItem( hwnd, CHK_FILL_SCREEN ), false );
        ScrollBar_Enable( GetDlgItem( hwnd, SCR_RENDER_RES ), ESB_DISABLE_BOTH );
        Static_Enable( GetDlgItem( hwnd, STC_RENDER_RES ), false );
    }
    ...
    // Set the Use Render Res check box
    Button_SetCheck( GetDlgItem( hwnd, CHK_USE_SCREEN_RES ), Settings.GetUseScreenRes() );
	...
	// Set the shading radio buttons
	if( Settings.GetShadeType() == EShade_FLAT )
		Button_SetCheck( GetDlgItem( hwnd, RDO_FLAT_SHADE ), TRUE );
	else
	{
		Button_SetCheck( GetDlgItem( hwnd, RDO_SMOOTH_SHADE ), TRUE );
		Button_Enable( GetDlgItem( hwnd, CHK_SHIMMER_QUOTE ), TRUE );
	}

	// Is shimmer turned on
	Button_SetCheck( GetDlgItem( hwnd, CHK_SHIMMER_QUOTE ), Settings.GetShimmer() );
...
void TConfigDlg::OnCommand( HWND hwnd, int id, HWND hwndCtl, UINT codeNotify )
{
    switch(id)
    {
		...
		case RDO_FLAT_SHADE:
			Button_Enable( GetDlgItem( hwnd, CHK_SHIMMER_QUOTE ), FALSE );
		break;

		case RDO_SMOOTH_SHADE:
			Button_Enable( GetDlgItem( hwnd, CHK_SHIMMER_QUOTE ), TRUE );
		break;
        ...
        case CHK_USE_SCREEN_RES:
            if( Button_GetCheck( GetDlgItem( hwnd, CHK_USE_SCREEN_RES ) ) )
			{
                Button_Enable( GetDlgItem( hwnd, CHK_FILL_SCREEN ), false );
                ScrollBar_Enable( GetDlgItem( hwnd, SCR_RENDER_RES ), ESB_DISABLE_BOTH );
                Static_Enable( GetDlgItem( hwnd, STC_RENDER_RES ), false );
            ...
            {
                Button_Enable( GetDlgItem( hwnd, CHK_FILL_SCREEN ), true );
                ScrollBar_Enable( GetDlgItem( hwnd, SCR_RENDER_RES ), ESB_ENABLE_BOTH );
                Static_Enable( GetDlgItem( hwnd, STC_RENDER_RES ), true );
            }
...
void TConfigDlg::RecordUserSettings( HWND hwnd )
{
    ...
    Settings.SetUseScreenRes( Button_GetCheck( GetDlgItem( hwnd, CHK_USE_SCREEN_RES ) ) );

    // Save the random music settings
    Settings.SetRandomMusic( Button_GetCheck( GetDlgItem( hwnd, CHK_RAND_MUSIC ) ) );

	// Save the shading settings
	if( Button_GetCheck( GetDlgItem( hwnd, RDO_FLAT_SHADE ) ) )
		Settings.SetShadeType( EShade_FLAT );
	else
		Settings.SetShadeType( EShade_GOURAUD );

	Settings.SetShimmer( Button_GetCheck( GetDlgItem( hwnd, CHK_SHIMMER_QUOTE ) ) );

}   // RecordUserSettings


/************************************************************************
*    FUNCTION NAME:         SetQuoteColor                                                             
*
*    DESCRIPTION:           Set the quote color
*
*    FUNCTION PARAMETERS:
*    Input:    HWND hwnd    - Handle of the window receiving the message 
*
...
void TConfigDlg::SetQuoteColor( HWND hwnd )
{
    CHOOSECOLOR cc;
    ...
    if( ChooseColor( &cc ) )
    {
        ...
        Settings.SetQuoteColor( cc.rgbResult );
    }
...
/************************************************************************
*    FUNCTION NAME:         SetRefColor                                                             
*
*    DESCRIPTION:           Set the ref color
*
*    FUNCTION PARAMETERS:
*    Input:    HWND hwnd    - Handle of the window receiving the message 
*
...
void TConfigDlg::SetRefColor( HWND hwnd )
{
    CHOOSECOLOR cc;
    ...
    {
        // Save the selected color
        Settings.SetRefColor( cc.rgbResult );
    }
...
BOOL GetWidthHeightCamera( int ResID, int &ScreenW, int &ScreenH, int &CameraSize )
{
    ...
    switch( ResID )
    {
        case RES_320x200:
            ...
            ScreenW = 320; ScreenH = 200; CameraSize = 160;
            Result = TRUE;
        break;

        case RES_400x300:

            ScreenW = 400; ScreenH = 300; CameraSize = 220;
            Result = TRUE;
        break;

        case RES_512x384:

            ScreenW = 512; ScreenH = 384; CameraSize = 285;
            Result = TRUE;
        break;

        case RES_640x480:

            ScreenW = 640; ScreenH = 480; CameraSize = 360;
            Result = TRUE;
        break;
    
        case RES_800x600:

            ScreenW = 800; ScreenH = 600; CameraSize = 450;
            Result = TRUE;
        break;
    
        case RES_1024x768:

            ScreenW = 1024; ScreenH = 768; CameraSize = 580;
            Result = TRUE;
        break;  

        case RES_1152x864:

            ScreenW = 1152; ScreenH = 864; CameraSize = 650;
            Result = TRUE;
        break;

        case RES_1280x720:

            ScreenW = 1280; ScreenH = 720; CameraSize = 600;
            Result = TRUE;
        break;

        case RES_1280x960:

            ScreenW = 1280; ScreenH = 960; CameraSize = 700;
            Result = RES_1280x960;
        break;

        case RES_1280x1024:

            ScreenW = 1280; ScreenH = 1024; CameraSize = 730;
            Result = TRUE;
        break;

        case RES_1600x900:

            ScreenW = 1600; ScreenH = 900; CameraSize = 700;
            Result = TRUE;
        break;

        case RES_1600x1024:

            ScreenW = 1600; ScreenH = 1024; CameraSize = 800;
            Result = TRUE;
        break;

        case RES_1600x1200:

            ScreenW = 1600; ScreenH = 1200; CameraSize = 900;
            Result = TRUE;
        break;
    }
