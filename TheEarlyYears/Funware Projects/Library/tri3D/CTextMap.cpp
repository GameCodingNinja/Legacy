...
bool CTextureMap::LoadFromFile( char *fileName, EBT_FLIP_FLOP flipFlop )
{
    ...
    // We don't support anyhting less then 16 bit bmp files.
	...
	// If 16 bit color, read in and convert
	if( infoHeader.biBitCount == 16 )
    {
        // Load the bitmap
        for( y = 0; y < height; ++y )
        {
            // Depending on how the uv is setup, we may need to flip
			// the bitmap or not.
            if( flipFlop == EBT_KEEP_BOTTOM_TO_SAME )
            ...
            if( fread( &pImage16[ posY ], width * 2, 1, hFile ) != 1 )
                goto HANDLE_ERROR;
        }
    }
    ...
    // If 32 bit color, read in and convert
    else if( infoHeader.biBitCount == 32 )
    {
        ...
        for( y = 0; y < height; ++y )
        {
            ...
            // Just a simple copy over
            if( fread( &pImage32[ posY ], width * 4, 1, hFile ) != 1 )
                goto HANDLE_ERROR;
        }
    }
...
bool CTextureMap::Create16BitTextFrom32()
{
    ...
    // We may have already loaded in a 16 bit image
    if( pImage16 == NULL )
	{
