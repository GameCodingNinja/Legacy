...
extern unsigned short int *gusiVBufer;
...
void _fastcall CTriangle::CalcLightIntenStatic( CLightList *pLightList )
{
...
void _fastcall CTriangle::CalcLightIntenDynamic( CLightList *pLightList )
{
    int vertInten, i[3]={0,0,0};
    double cosAngle, dist;
    ...
    for( int lite = 0; lite < pLightList->count; ++lite )
    {
        for( int vIndex = 0; vIndex < 3; ++vIndex )
        {
            ...
            {    
                ...
                {                                                        
                    // Assign an intensity based on the vertex surface normal
                    cosAngle = pLightList->light[lite].CalAngleToSurfaceDy( uvi[ vIndex ].n, pVPoint[vIndex] );
                }
                else if( pLightList->light[lite].shadeType == EShade_FLAT && vIndex == 0 )
                {
                    // Assign an intensity based on the vertex surface normal
            ...
            {
                // Get the direction of the light
                if( pLightList->light[lite].CalDirToSurfaceDy( uvi[ vIndex ].n ) > 0 )
                {                   
                    dist = pLightList->light[lite].CalDistToSurfaceDy( pVPoint[vIndex] );
...
*                           this triangle is used to check for collision
...
int _fastcall CTriangle::Collide_BoundSphere( CPoint &pt, double &distance, float rad )
{
    float x, y, z;
    double dist;

    // Get the center of the triangle
    x = (pVPoint[0]->tx + pVPoint[1]->tx + pVPoint[2]->tx) * 0.33333333;
    y = (pVPoint[0]->ty + pVPoint[1]->ty + pVPoint[2]->ty) * 0.33333333;
    z = (pVPoint[0]->tz + pVPoint[1]->tz + pVPoint[2]->tz) * 0.33333333;

    // Calculate the distance between the center points of both objects
    dist = sqrt( (( x - pt.tx ) * ( x - pt.tx )) +
                 (( y - pt.ty ) * ( y - pt.ty )) +
                 (( z - pt.tz ) * ( z - pt.tz )) );
    ...
    if( dist <= rad + radius )
    {
        // Record the distance for comparison
        distance = fabs( dist );
...
void _fastcall CTriangle::ClipProjectUVI()
{
    for( int i = 0; i < projCount; ++i )
    {
        ...
        if( point2D[i].fClipped )
        {
            ...
            float I (( double ) uvi[ startI ].i + ( double ) 
            ...
            point2D[i].I = I * FIX_SHIFT_15_FLOAT;
        }
        else
        {
            // The 1/Z is needed for perspective calculations
            ...
            // Gouraud shading doesn't use perspective. It's straight interpolation.
            point2D[i].I = uvi[endI].i * FIX_SHIFT_15_FLOAT;
        }
    }
...
void _fastcall CTriangle::ClipProjectI()
{
    for( int i = 0; i < projCount; ++i )
    {
        ...
        {
            // Gouraud shading doesn't use perspective. It's straight interpolation.
            point2D[i].I = uvi[ point2D[i].endI ].i * FIX_SHIFT_15_FLOAT;
        }
    }
...
bool _fastcall CTriangle::CalcVisible3D( float cullPercent )
{
    ...
    // Do the pre-calc
    float z_test = farPoint * gfSquarePercentage;
    ...
    z_test = farPoint * gfFrustrumYRatio;
...
void _fastcall CTriangle::Render16_WireFrame( PCTextureLib pTextLib )
{
    ...
    unsigned short int register *pVidBuf = gusiVBufer;
    unsigned short int register color = pTextLib->pTMap[tIndex].pImage16[130];
    unsigned short int register *pShade = gusiShadeTbl;
...
/************************************************************************
*    FUNCTION NAME:         Render_ZBufShadingTxtPos()
*
...
void _fastcall CTriangle::Render_ZBufShadingTxtPos( PCTextureLib pTextLib )
{
    ...
    int register *ZPtr;
    unsigned short int *tmpVBuf = gusiVBufer;
                ...
                for( int count = 0; count < runLoops; ++count )
                {
                    ...
                    while( length > 0 )
                    {
                        ...
                        if( *ZPtr < Z )
                        {
                            ...
                            *DPtr = pShade[ ( I & SHIFT_16_MASK ) + pText[ (( Ty & SHIFT_23_MASK ) + Tx) >> shiftXaddition ]];                            
                        }
...
}   // Render_ZBufShadingTxtPos


/************************************************************************
*    FUNCTION NAME:         Render_ZBufShadingTxtNeg()
*
*    DESCRIPTION:           Grey shading.
*
*    FUNCTION PARAMETERS:
...
void _fastcall CTriangle::Render_ZBufShadingTxtNeg( PCTextureLib pTextLib )
{
    CCeilLine leftSeg, rightSeg;
    int top(0), yIndex, height, xStart, xEnd, width;
    int newRightPos, newLeftPos, runLoops, length;
...
}   // Render_ZBufShadingTxtNeg


/************************************************************************
*    FUNCTION NAME:         Render16_LiteZBufShadingPos()
*
...
*
*    DESCRIPTION:           Get the a specific point of the triangle
*
*    FUNCTION PARAMETERS:
...
void CTriangle::CopyPoint( CPoint &pt, int index )
{
...
void CTriangle::CopyNormal( CPoint &pt )
{
    pt.x = pNormal->x;
    pt.y = pNormal->y;
    pt.z = pNormal->z;
