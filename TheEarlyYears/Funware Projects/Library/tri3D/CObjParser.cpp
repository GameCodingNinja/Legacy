...
#include "CObjParser.h"      // Header file for this *.cpp file.
#include "CMeshTri.h"        // mesh class
...
/************************************************************************
*    FUNCTION NAME:         SortByZCenters()                                                             
*
*    DESCRIPTION:           Sort all the polygons by their Z order in
*                           an assending order.
*
*    FUNCTION PARAMETERS:
...
void CObjParser::SortByZCenters( CObjList &tmpFaceLst )
{
    int offset, j, x=0;
    bool inOrder;
    ...
    if( offset > 1 )
    {
        while( x < offset && inOrder == false )
        {
            ...
            for( j = 0; j < offset - x; ++j )
            {
                ...
                if( pFace1->CalZCenter() > pFace2->CalZCenter() )
                {
                    inOrder = false;
                    ptemp = pFace1;
...
bool CObjParser::BuildTextureLibrary( CTextureLib &textLib, CObjList &noDupTxtLst, char *filePath )
{
    ...
    for( i = 0; i < textLib.Count(); ++i )
    {
        ...
        if( !textLib.AddTextureToLib( i, pathToBMP, EBT_BOTTOM_TOP_FLIP ) )
        {
...
void CObjParser::ConvertUVtoSpecificCoordinates( CTextureLib &textLib, CObjList &groupLst )
{
    ...
    {        
        ...
        for( j = 0; j < pGroup->faceLst.Count(); ++j )
        {
            ...
            for( w = 0; w < 3; ++w )
            {
                pFace->crd[w].uv[0] *= (textLib.pTMap[pFace->tIndex].width-1);
                pFace->crd[w].uv[1] *= (textLib.pTMap[pFace->tIndex].height-1);
            }
        }
    }
    ...
    {        
        ...
        for( j = 0; j < pGroup->faceLst.Count(); ++j )
        {
            ...
            for( w = 0; w < 3; ++w )
            {
                ...
                fprintf( hFile, "      UV %d: u:%f, v:%f\n", w+1, pFace->crd[w].uv[0], pFace->crd[w].uv[1] );
            }
