...
public:
    ...
    bool _fastcall CalcVisible3D();
    // Check if the poly is visible. Perform dD culling
    bool _fastcall CalcVisible2D();
    // Normalize the plain orientation
    void Normalize( CPoint &n, int vIndex );
    void Normalize();
    ...
    ///////////////////////////////////
    // Z Buffered rendering and RGB grouraud shading.
    void _fastcall Render32_ZBufRGBShading( PCTextureLib pTextLib );
    // RGB grouraud shading.
    void _fastcall Render32_RGBShading( PCTextureLib pTextLib );
    // Grey grouraud shading.
    void _fastcall Render32_GreyShading( PCTextureLib pTextLib );
    // No shading.
    void _fastcall Render32_NoShading( PCTextureLib pTextLib );
    // Z Buffered rendering and Grey grouraud shading.
    void _fastcall Render32_ZBufGreyShading( PCTextureLib pTextLib );
    // Z Buffered rendering and No grouraud shading.
    void _fastcall Render32_ZBufNoShading( PCTextureLib pTextLib );
	// RGB grouraud shading, solid color.
    void _fastcall RenderSolid32_LiteZBufRGBShading();
...
{
public:
    int X, Y, Z, U, V, I, rI, gI, bI, x1, x2, y1, y2;
    int stepX, stepZ, stepIr, stepIg, stepIb, stepI, stepU, stepV, edgeHeight;

    // The constructor
    CCeilLine():x1(0), x2(0), y1(0), y2(0), edgeHeight(0), X(0), Y(0), Z(0),
    rI(0), gI(0), bI(0), U(0), V(0), stepX(0), stepZ(0), stepU(0), stepV(0),
	stepIr(0), stepIg(0), stepIb(0), I(0), stepI(0)
    {
    }
	...
	inline void CCeilLine::Init( CPoint2D &point2D_Pos, CPoint2D &point2D_newPos )
	{
	    ...
	    if( edgeHeight )
	    {
	        ...
	        stepIr = ( point2D_newPos.rI - point2D_Pos.rI ) / edgeHeight;
			stepIg = ( point2D_newPos.gI - point2D_Pos.gI ) / edgeHeight;
			stepIb = ( point2D_newPos.bI - point2D_Pos.bI ) / edgeHeight;
