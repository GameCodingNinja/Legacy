...
************************************************************************/
void CPolygon::ClipProject( CTextureLib *pTextLib )
{
    int inc, start;
    ...
    for( int i = 0; i < projCount; ++i )
    {
        ...
        projectPoint[i].U = ((clipUVI[i].u * (pTextLib->pTMap[tIndex].GetWidth()-1)) * oneOverZ) * FIX_SHIFT_26_FLOAT;
        projectPoint[i].V = ((clipUVI[i].v * (pTextLib->pTMap[tIndex].GetHeight()-1)) * oneOverZ) * FIX_SHIFT_26_FLOAT;
...
*
*    -------------------------------------------------------------------
*    H Rosenorn    03/03/05      Function Created
*    -------------------------------------------------------------------
*    UserID        MM/DD/YY      Description of any changes
************************************************************************/
void _fastcall CPolygon::RenderClipped( CTextureLib *pTextLib )
{
    ...
    unsigned int shiftXaddition = FIX_SHIFT_23_INT - pTextLib->pTMap[tIndex].GetMultiShift();
    unsigned char shiftXadditionByte = (unsigned char)shiftXaddition;

    #ifndef _16_BIT_SOFTWARE_RENDERING_
        unsigned int register *DPtr, *pText;
        pText = (unsigned int *)pTextLib->pTMap[tIndex].GetBits();
        unsigned int textel;
        unsigned int *tmpVBuf = (unsigned int *)gusiVBufer;
    #else
        unsigned short int register *DPtr, *pText, *pShade;
        pText = (unsigned short int *)pTextLib->pTMap[tIndex].GetBits();
