...
void CMapMgr<mesh_type>::RenderRoom( CMatrix &m, char *debug )
{
    ...
    // get the objects attached to this room
    CObjLst<CMeshMap> *pObjLst = &roomLst.Get( activeIndex )->objLst;

    // Render all the objects in this room
	RenderObjects( m, &roomLst.Get( activeIndex )->objLst );
    ...
    for( int i = 0; i < pObjLst->Count(); ++i )
    {
        ...
        if( pObjLst->Get(i)->GetType() == EOT_MAP_COLLISION )
        {
            pObjLst->Get(i)->Transform( m );
        }
        else  // Render the rest of the objects
        {
            totalCount += pObjLst->Get(i)->Render( m );
        }
    }
    ...
    for( int i = 0; i < pDoorLst->Count(); ++i )
    {
        ...
        if( !pDoorMesh->WasRendered() && !pDoorMesh->CullMesh_BoundSphere() )
        {
            ...
            // get the objects attached to this room
            pObjLst = &roomLst.Get( activeIndex )->roomDoorLst.Get( i )->objLst;                
            ...
            if( pDoorMesh->GetDistance() > pDoorMesh->GetRadius() )
            {
                ...
                for( int i = 0; i < pObjLst->Count(); ++i )
                {
                    ...
                    // Only render objects of no specific type
                    if( pObjMesh->GetType() == EOT_NO_TYPE )
                    {
                        totalCount += pObjMesh->Render( m, pDoorMesh->GetRadius(),
            ...
            else
            {
                ...
                for( int i = 0; i < pObjLst->Count(); ++i )
                {
                    ...
                    if( pObjMesh->GetType() == EOT_NO_TYPE )
                    {
                        totalCount += pObjMesh->Render( m, !pObjMesh->WasTransformed() );
                    }
                }
            ...
            for( int j = 0; j < pInnerDoorLst->Count(); ++j )
            {
                ...
                if( !innerDoorMesh->WasRendered() )
                {
                    ...
                    if( !innerDoorCulled )
                    {
                        ...
                        {
                            CMeshMap *pInnerRoomMesh = roomLst.Get( innerDoorMesh->GetAttachedRoom(w) )->pRoom;
...
void CMapMgr<mesh_type>::RenderDoor( CMatrix &m, char *debug )
{
    ...
    {
        // get the objects attached to this room
        CObjLst<CMeshMap> *pObjLst = &doorLst.Get( activeIndex )->roomDoorLst.Get( i )->objLst;

        // Render all the objects in this room
        for( int obj = 0; obj < pObjLst->Count(); ++obj )
        {
            // Only transform the objects we can see
            if( pObjLst->Get(obj)->GetType() != EOT_MAP_COLLISION )
            {
                totalCount += pObjLst->Get(obj)->Render( m );
            }
        }
        ...
        for( int j = 0; j < pDoorLst->Count(); ++j )
        {
            ...
            if( !pDoorMesh->WasCullTested() && !pDoorMesh->WasRendered() )
            {
                ...
                if( !pDoorMesh->CullMesh_BoundSphere() )
                {
                    ...
                    for( int w = 0; w < MAX_ROOMS_PER_DOOR; ++w )
                    {
                        CMeshMap *pInnerRoomMesh = roomLst.Get( pDoorMesh->GetAttachedRoom(w) )->pRoom;
                        ...
                        // get the objects attached to this room
                        pObjLst = &roomLst.Get( pDoorMesh->GetAttachedRoom(w) )->objLst;

                        // Render all the objects in this room
                        for( int obj = 0; obj < pObjLst->Count(); ++obj )
                        {
                            CMeshMap *pObjMesh = pObjLst->Get(obj);
                            totalCount += pObjMesh->Render( m, pDoorMesh->GetRadius(),
                                                            pDoorMesh->GetCenterPt(),
                                                            !pObjMesh->WasTransformed() );
                        }
...
int CMapMgr<mesh_type>::RenderObjects( CMatrix &m, CObjLst<CMeshMap> *pObjLst,
                                       bool transformCol, CMeshMap *pMeshPortal )
{
	int count = 0;
	...
	if( pMeshPortal == NULL )
	{
	    ...
	    {
	        // Only transform the collision object because it is not rendered.
	        if( pObjLst->Get(i)->GetType() == EOT_MAP_COLLISION && transformCol )
	        {
	            pObjLst->Get(i)->Transform( m );
	        }
	        else  // Render the rest of the objects
	        {
	            count += pObjLst->Get(i)->Render( m );
	        }
	    }
	}
	else
...
void CMapMgr<mesh_type>::SetupMap()
{
    ...
    for( int i = 0; i < meshCount; ++i )
    {
        if( pMesh[i].GetType() == EOT_NO_TYPE ||
            pMesh[i].GetType() == EOT_MAP_COLLISION )
        {
            roomLst.Get( pMesh[i].GetAttachedRoom( 0 ) )->objLst.Add( &pMesh[i] );
        ...
        for( int j = 0; j < doorLst.Count(); ++j )
        {
            for( int w = 0; w < MAX_ROOMS_PER_DOOR; ++w )
            {
                ...
                if( doorLst.Get( j )->pDoor->GetAttachedRoom( w ) == i )
                {
                    ...
                    for( int k = 0; k < roomLst.Get( doorLst.Get( j )->pDoor->GetAttachedRoom( !w ) )->objLst.Count(); ++k )
                    {
                        pRoomDoors->objLst.Add( roomLst.Get( doorLst.Get( j )->pDoor->GetAttachedRoom( !w ) )->objLst.Get( k ) );
                    }
    ...
    for( int i = 0; i < doorLst.Count(); ++i )
    {
        for( int j = 0; j < MAX_ROOMS_PER_DOOR; ++j )
        {
            ...
            // Add the objects to the list
            for( int k = 0; k < roomLst.Get( doorLst.Get( i )->pDoor->GetAttachedRoom( j ) )->objLst.Count(); ++k )
            {
                pRoomDoors->objLst.Add( roomLst.Get( doorLst.Get( i )->pDoor->GetAttachedRoom( j ) )->objLst.Get( k ) );
            }
