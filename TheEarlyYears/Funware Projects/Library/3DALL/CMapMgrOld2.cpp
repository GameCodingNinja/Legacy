...
void CMapMgr<mesh_type>::Render( CMatrix &m, char *debug )
{
    ...
    int roomCount = 0;
    int totalCount = 0;
    CMeshMap *pDoorMesh, *pRoomMesh, *innerDoorMesh, *pInnerRoomMesh, *pObjMesh;
    CObjLst<CMeshMap> *pDoorLst, *pInnerDoorLst, *pObjLst;
    ...
    if( activeType == EOT_MAP_ROOM )
    {
        ...
        {
            totalCount += roomLst.Get( activeIndex )->objLst.Get(i)->Render( m );
        }
                        ...
                        if( !innerDoorMesh->WasTransformed() )
                        {
                            ...
                            if( !closeToDoorWay )
                            {
                                totalCount += innerDoorMesh->Render( m, pDoorMesh->GetRadius(),
                                                                     pDoorMesh->GetCenterPt(),
                                                                     !innerDoorMesh->WasTransformed() );
                            }
                            else
                            {
                                totalCount += innerDoorMesh->Render( m, !innerDoorMesh->WasTransformed() );
                            }
    ...
    {        
        ...
        {
            // get the objects attached to this room
            pObjLst = &doorLst.Get( activeIndex )->roomDoorLst.Get( i )->objLst;
            ...
            for( int j = 0; j < pDoorLst->Count(); ++j )
            {
                ...
                if( !pDoorMesh->WasCullTested() && !pDoorMesh->WasRendered() )
                {
                    ...
                    if( !pDoorMesh->CullMesh_BoundSphere() )
                    {
                        ...
                        for( int w = 0; w < MAX_ROOMS_PER_DOOR; ++w )
                        {
                            pInnerRoomMesh = roomLst.Get( pDoorMesh->GetAttachedRoom(w) )->pRoom;
