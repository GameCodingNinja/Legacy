...
protected:
    ...
    CPoint2D projectPoint[ MAX_CLIPPED_POINTS ];
    // Projection count
    int projCount;
    // clipped flag
    bool clipped;
...
public:
    ...
    virtual bool _fastcall IsVisable();
    ...
    void _fastcall RenderDirectXText( PCTextureLib pTextLib );

    // Software rendering functions
    void ClipProject( CTextureLib *pTextLib );
    void ClipProject();
    void _fastcall Render_WireFrame();
    void _fastcall RenderClipped( PCTextureLib pTextLib );
    void _fastcall Render( PCTextureLib pTextLib );
