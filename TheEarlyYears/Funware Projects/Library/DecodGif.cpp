...
BYTE * TGif::LoadFromFile( char *name )
{
    return DecodeGif( name );
}

// Wrapper to help keep it simple and more readable
BYTE * TGif::LoadFromResource( char *name, char *type, HINSTANCE hInst )
{
    return DecodeGif( name, type, hInst );
}

// Wrapper to help keep it simple and more readable
BYTE * TGif::LoadFromBuffer( BYTE *CmpBuffer, int Size )
{
    size = Size;
...
BYTE * TGif::DecodeGif( char *name, char *type, HINSTANCE hInst, BYTE *CmpBuffer )
{
    int            numcolors;
    int            ImageBitCount;
...
#if GET_BYTES
unsigned int TGif::get_bytes(unsigned int count, BYTE *dest)
{
...
int TGif::decoder(int linewidth)
{
#if NEAR_STACK
    register BYTE   *sp;
...
void TGif::putcolor(int x, int y, int color)
{
    long i;
...
int TGif::put_line(int rownum, int leftpt, int rightpt, unsigned char *localvalues)
{
    int i, len;
    long    startloc;
...
int TGif::out_line(unsigned char *localvalues, int numberofdots)
{
