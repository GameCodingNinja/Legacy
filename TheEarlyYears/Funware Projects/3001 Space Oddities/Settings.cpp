...
void TSettings::GetPrivateString()
{
    ...
    // Get the index to the number of stars needed
    StarCount = GetSettingsInt( IniFilePath, PRODUCT_NAME, "StarCount", "1" );
    // Do we show the intro window
    IntroHelpWnd = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Intro Help", "1" );
    // Get the movement of the stars. 5 will randomize all four options
    StarMovement = GetSettingsInt( IniFilePath, PRODUCT_NAME, "Movement", "4" );
    // Do we show the planets
    PlanetsOnOff = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Planets", "1" );
    // Do we show the Space Ships
    SpaceShipsOnOff = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Space Ships", "1" );
    // Do we want the space clouds
    CloudsOnOff = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Space Clouds", "1" );
    ...
    // Do we the planets infront of the stars
    PlanetsInfrontOfStars = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Planet Positions", "0" );
    // Get the number of planets on screen at any given moment
    NumPlanetsAtOneTime = GetSettingsInt( IniFilePath, PRODUCT_NAME, "Planets on screen", "1" );
    // Get the number of ships on screen at any given moment
    NumShipsAtOneTime = GetSettingsInt( IniFilePath, PRODUCT_NAME, "Ships on screen", "1" );
...
void TSettings::WritePrivateString()
{
    ...
    // Save the index to the number of stars needed
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "StarCount", StarCount );
    // Get the movement of the stars
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Movement", StarMovement );
    // Save if we show the planets
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Planets", PlanetsOnOff );
    // Save if we show the Space Ships
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Space Ships", SpaceShipsOnOff );
    // Save the sound directory
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Sound Dir", SoundDirectory );
    // Save the space clouds
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Space Clouds", CloudsOnOff );
    // Save randomize the music
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Randomize Music", RandomMusic );
    // Save if the planets are in front of the stars
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Planet Positions", PlanetsInfrontOfStars );
    // Save the number of planets on screen at any given moment
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Planets on screen", NumPlanetsAtOneTime );
    // Save the number of ships on screen at any given moment
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Ships on screen", NumShipsAtOneTime );
