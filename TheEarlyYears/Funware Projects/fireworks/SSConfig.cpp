...
BOOL TConfigDlg::OnInitDialog( HWND hwnd, HWND hwndFocus, LPARAM lParam )
{
    ...
    // Setup the Firework count scroll bar
    ScrollBar_SetRange( GetDlgItem( hwnd, SCR_FIREWORK_COUNT ), 1, MAX_FIREWORKS, TRUE );
    ScrollBar_SetPos( GetDlgItem( hwnd, SCR_FIREWORK_COUNT ), Settings.GetFireworkCount(), TRUE );
    SetScrollBarLabels( hwnd, GetDlgItem( hwnd, SCR_FIREWORK_COUNT ), Settings.GetFireworkCount() );

    // Setup the view angle
    ScrollBar_SetRange( GetDlgItem( hwnd, SCR_BURST_ANGLE ), 0, MAX_ANGLES-1, TRUE );
    ScrollBar_SetPos( GetDlgItem( hwnd, SCR_BURST_ANGLE ), Settings.GetBurstAngle(), TRUE );
    SetScrollBarLabels( hwnd, GetDlgItem( hwnd, SCR_BURST_ANGLE ), Settings.GetBurstAngle() );
...
void TConfigDlg::SetScrollBarLabels( HWND hwnd, HWND hScrollBar, int ScrollPos )
{
    ...
    else if( hScrollBar == GetDlgItem( hwnd, SCR_FIREWORK_COUNT ) )
    {
        // Set the string
        wsprintf( strTmp, "Maximum Firework Bursts: %d", ScrollPos );

        Static_SetText( GetDlgItem( hwnd, STC_FIREWORK_COUNT ), strTmp );
    }
    else if( hScrollBar == GetDlgItem( hwnd, SCR_BURST_ANGLE ) )
    {
        switch( ScrollPos )
        {
            case ANGLE_20_DEGREES:
                Static_SetText( GetDlgItem( hwnd, STC_BURST_ANGLE ), "Burst Angle: 20 Degree View" );
            break;

            case ANGLE_50_DEGREES:
                Static_SetText( GetDlgItem( hwnd, STC_BURST_ANGLE ), "Burst Angle: 50 Degree View" );
            break;

            case ANGLE_90_DEGREES:
                Static_SetText( GetDlgItem( hwnd, STC_BURST_ANGLE ), "Burst Angle: 90 Degree View" );
            break;
        }       
    }
...
void TConfigDlg::RecordUserSettings( HWND hwnd )
{
    ...
    // Save the position of the fireworks count
    Settings.SetFireworkCount( ScrollBar_GetPos( GetDlgItem( hwnd, SCR_FIREWORK_COUNT ) ) );

    // Save the view angle
    Settings.SetBurstAngle( ScrollBar_GetPos( GetDlgItem( hwnd, SCR_BURST_ANGLE ) ) );
...
DWORD GetRenderTime( int ResID, HINSTANCE hInst )
{
    ...
    if( !LoadTheResource( hInst, "ShadedTable8bit", "STBL", &LoadRes ) )
        goto HANDLE_ERROR;
    ...
    for( i = 0; i < 5; ++i )
    {
        ...
        if( i % 2 )
            testPoly.PaintVBuffer8To8( TimeTest, POS_ZBUFFER );
        else
            testPoly.PaintVBuffer8To8( TimeTest, NEG_ZBUFFER );
