...
void TSettings::GetPrivateString()
{
    ...
    *****************************************/
	...
	PolyRes = GetSettingsInt( IniFilePath, PRODUCT_NAME, "Poly Res", "0" );
	// Get the shading type
	ShadeType = GetSettingsInt( IniFilePath, PRODUCT_NAME, "Shade Type", "0" );
    // Get the custom texture file
    lstrcpy( LoadCustomTxt, GetSettingsChar( IniFilePath, PRODUCT_NAME, "Load Custom Texture", "" ) );
    // Get the custom texture path
    lstrcpy( CustomPath, GetSettingsChar( IniFilePath, PRODUCT_NAME, "Custom Path", "" ) );

    // Get the render resolution
    RenderRes = GetSettingsInt( IniFilePath, PRODUCT_NAME, "Render Res", "3" );
    // Get Size to fit
    SizeToFit = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Size To Fit", "1" );
    // Did we calculate the render res
    CalcRenderRes = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Cal Render Res", "1" );
    // Do we want to render in 16 bit color
    Force16BitColor = GetSettingsBool( IniFilePath, PRODUCT_NAME, "Force 16 Bit Color", "0" );
...
void TSettings::WritePrivateString()
{
    ...
    *****************************************/
    ...
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Poly Res", PolyRes );
    // Save the shading type
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Shade Type", ShadeType );
    // Save the custom texture
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Load Custom Texture", LoadCustomTxt );
    // Save the custom texture path
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Custom Path", CustomPath );

    // Save the render resolution
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Render Res", RenderRes );
    // Save the Size To Fit
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Size To Fit", SizeToFit );
    // Save cal render res
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Cal Render Res", CalcRenderRes );
    // Save 16 bit render depth
    SaveIniSetting( IniFilePath, PRODUCT_NAME, "Force 16 Bit Color", Force16BitColor );
