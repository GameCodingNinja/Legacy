...
// This array is padded with the same resolutions
// so that both arrays are the same
int ResCheck[MAX_RES_CHECK] = { RES_320x200,
								RES_512x384,
								RES_640x480,
								RES_800x600,
								RES_1024x768,
...
int ResCheckWide[MAX_RES_CHECK] = { RES_960x600,
								    RES_1024x600,
								    RES_1024x640,
									RES_1088x612,
								    RES_1280x720,
...
int BlitToScreen[MAX_RES_CHECK] = { 0, 0, 0, 2, 4, 6, 8, 8, 8, 8, 8, 8, 8, 8 };
...
BOOL TConfigDlg::OnInitDialog( HWND hwnd, HWND hwndFocus, LPARAM lParam )
{
    ...
    // Set the skip into
    Button_SetCheck( GetDlgItem( hwnd, CHK_JUMP_TO_BATTLE ), Settings.GetSkipIntro() );
...
void TConfigDlg::SetScrollBarLabels( HWND hwnd, HWND hScrollBar, int ScrollPos )
{
    ...
    if( hScrollBar == GetDlgItem( hwnd, SCR_RENDER_RES ) )
    {
        ...
        switch( ScrollPos )
        {
            ...
            break;
			case RES_960x600: 
                Static_SetText( hControl, "Render Resolution: 960 x 600" );
            break;
			case RES_1024x600: 
                Static_SetText( hControl, "Render Resolution: 1024 x 600" );
            break;
			case RES_1024x640: 
                Static_SetText( hControl, "Render Resolution: 1024 x 640" );
            break;
            case RES_1024x768: 
                Static_SetText( hControl, "Render Resolution: 1024 x 768" );
            break;
			case RES_1088x612: 
                Static_SetText( hControl, "Render Resolution: 1088 x 612" );
            break;
            case RES_1152x864: 
            ...
            break;
			case RES_1280x768: 
                Static_SetText( hControl, "Render Resolution: 1280 x 768" );
            break;
			case RES_1280x800: 
                Static_SetText( hControl, "Render Resolution: 1280 x 800" );
            break;
            case RES_1280x960: 
            ...
            break;
			case RES_1360x768: 
                Static_SetText( hControl, "Render Resolution: 1360 x 768" );
            break;
			case RES_1440x900: 
                Static_SetText( hControl, "Render Resolution: 1440 x 900" );
            break;
            case RES_1600x900: 
            ...
            break;
			case RES_1680x1050: 
                Static_SetText( hControl, "Render Resolution: 1680 x 1050" );
            break;
			case RES_1800x1440: 
                Static_SetText( hControl, "Render Resolution: 1800 x 1440" );
            break;
			case RES_1920x1080: 
                Static_SetText( hControl, "Render Resolution: 1920 x 1080" );
            break;
			case RES_1920x1200: 
                Static_SetText( hControl, "Render Resolution: 1920 x 1200" );
            break;
			case RES_2048x1536: 
                Static_SetText( hControl, "Render Resolution: 2048 x 1536" );
            break;
			case RES_3200x2400: 
                Static_SetText( hControl, "Render Resolution: 3200 x 2400" );
            break;
			case RES_6400x4800: 
                Static_SetText( hControl, "Render Resolution: 6400 x 4800" );
            break;
        }
    }
}   // SetScrollBarLabels
...
void TConfigDlg::RecordUserSettings( HWND hwnd )
{
    ...
    // Save the skip intro
    Settings.SetSkipIntro( Button_GetCheck( GetDlgItem( hwnd, CHK_JUMP_TO_BATTLE ) ) );
...
/************************************************************************
*    FUNCTION NAME:         DoResCheck                                                             
*
*    DESCRIPTION:           Do the resolution check
*
*    FUNCTION PARAMETERS:
*    Input:    TSettings & settings     - Active settings class
*
*    Output:   returntype - Description
*
*    HISTORY:
...
void DoResCheck( TSettings & Settings, HINSTANCE hInst )
{
	DWORD RenderTime;
    int ScreenResID;
    ...
    {        
        int * tmpResCheck = ResCheck;

		if( ((float)GetScreenWidth() / (float)GetScreenHeight()) > 1.4f )
		{
			tmpResCheck = ResCheckWide;
		}
        
        for( int i = 0; i < MAX_RES_CHECK; ++i )
        {
            RenderTime = GetRenderTime( tmpResCheck[i], hInst );

            // Make sure we at least set the first resolution if there wasn't an error
            if( RenderTime <= (DWORD)(MAX_MIL_RENDER_WAIT - BlitToScreen[i]) )
                Settings.SetRenderRes( tmpResCheck[i] );
            else
                break;
        }
...
int GetScreenResSizeID( int ScreenW, int ScreenH )
{
    ...
    if( ScreenW >= 960 && ScreenH >= 600 )
        Result = RES_960x600;

    if( ScreenW >= 1024 && ScreenH >= 600 )
        Result = RES_1024x600;

    if( ScreenW >= 1024 && ScreenH >= 640 )
        Result = RES_1024x640;
        
    if( ScreenW >= 1024 && ScreenH >= 768 )
        Result = RES_1024x768;
        
    if( ScreenW >= 1088 && ScreenH >= 612 )
        Result = RES_1088x612;  
    ...
    if( ScreenW >= 1360 && ScreenH >= 768 )
        Result = RES_1360x768;

    if( ScreenW >= 1440 && ScreenH >= 900 )
        Result = RES_1440x900;
	...
	if( ScreenW >= 1680 && ScreenH >= 1050 )
        Result = RES_1680x1050;

	if( ScreenW >= 1800 && ScreenH >= 1440 )
        Result = RES_1800x1440;

	if( ScreenW >= 1920 && ScreenH >= 1080 )
        Result = RES_1920x1080;

    if( ScreenW >= 1920 && ScreenH >= 1200 )
        Result = RES_1920x1200;

    if( ScreenW >= 2048 && ScreenH >= 1536 )
        Result = RES_2048x1536;

    if( ScreenW >= 3200 && ScreenH >= 2400 )
        Result = RES_3200x2400;

    if( ScreenW >= 4000 && ScreenH >= 3000 )
        Result = RES_4000x3000;

    if( ScreenW >= 6400 && ScreenH >= 4800 )
        Result = RES_6400x4800;
...
BOOL GetWidthHeightCamera( int ResID, int &ScreenW, int &ScreenH, int &CameraSize )
{
    ...
    switch( ResID )
    {
        case RES_320x200:
            ...
            ScreenW = 320; ScreenH = 200; CameraSize = 110;
            Result = TRUE;
        break;

        case RES_512x384:

            ScreenW = 512; ScreenH = 384; CameraSize = 190;
            Result = TRUE;
        break;

        case RES_640x480:

            ScreenW = 640; ScreenH = 480; CameraSize = 240;
            Result = TRUE;
        break;
    
        case RES_800x600:

            ScreenW = 800; ScreenH = 600; CameraSize = 300;
            Result = TRUE;
        break;

        case RES_960x600:

            ScreenW = 960; ScreenH = 600; CameraSize = 365;
            Result = TRUE;
        break;

        case RES_1024x600:

            ScreenW = 1024; ScreenH = 600; CameraSize = 375;
            Result = TRUE;
        break;

        case RES_1024x640:

            ScreenW = 1024; ScreenH = 640; CameraSize = 375;
            Result = TRUE;
        break;
    
        case RES_1024x768:

            ScreenW = 1024; ScreenH = 768; CameraSize = 385;
            Result = TRUE;
        break;
        
        case RES_1088x612:

            ScreenW = 1088; ScreenH = 612; CameraSize = 390;
            Result = TRUE;
        break;  

        case RES_1152x864:

            ScreenW = 1152; ScreenH = 864; CameraSize = 425;
            Result = TRUE;
        break;

        case RES_1280x720:

            ScreenW = 1280; ScreenH = 720; CameraSize = 450;
            Result = TRUE;
        break;

        case RES_1280x768:

            ScreenW = 1280; ScreenH = 768; CameraSize = 460;
            Result = TRUE;
        break;

        case RES_1280x800:

            ScreenW = 1280; ScreenH = 800; CameraSize = 465;
            Result = TRUE;
        break;

        case RES_1280x960:

            ScreenW = 1280; ScreenH = 960; CameraSize = 475;
            Result = RES_1280x960;
        break;

        case RES_1280x1024:

            ScreenW = 1280; ScreenH = 1024; CameraSize = 500;
            Result = TRUE;
        break;

        case RES_1360x768:

            ScreenW = 1360; ScreenH = 768; CameraSize = 520;
            Result = TRUE;
        break;

        case RES_1440x900:

            ScreenW = 1440; ScreenH = 900; CameraSize = 530;
            Result = TRUE;
        break;

        case RES_1600x900:

            ScreenW = 1600; ScreenH = 900; CameraSize = 550;
            Result = TRUE;
        break;

        case RES_1600x1024:

            ScreenW = 1600; ScreenH = 1024; CameraSize = 550;
            Result = TRUE;
        break;

        case RES_1600x1200:

            ScreenW = 1600; ScreenH = 1200; CameraSize = 600;
            Result = TRUE;
        break;

        case RES_1680x1050:

            ScreenW = 1680; ScreenH = 1050; CameraSize = 550;
            Result = TRUE;
        break;

        case RES_1800x1440:

            ScreenW = 1800; ScreenH = 1440; CameraSize = 600;
            Result = TRUE;
        break;

        case RES_1920x1080:

            ScreenW = 1920; ScreenH = 1080; CameraSize = 650;
            Result = TRUE;
        break;

        case RES_1920x1200:

            ScreenW = 1920; ScreenH = 1200; CameraSize = 700;
            Result = TRUE;
        break;

        case RES_2048x1536:

            ScreenW = 2048; ScreenH = 1536; CameraSize = 700;
            Result = TRUE;
        break;

        case RES_3200x2400:

            ScreenW = 3200; ScreenH = 2400; CameraSize = 700;
            Result = TRUE;
        break;

        case RES_4000x3000:

            ScreenW = 4000; ScreenH = 3000; CameraSize = 700;
            Result = TRUE;
        break;

        case RES_6400x4800:

            ScreenW = 6400; ScreenH = 4800; CameraSize = 700;
            Result = TRUE;
        break;
    }
