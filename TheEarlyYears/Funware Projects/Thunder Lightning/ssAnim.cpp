...
DWORD flashTimeOutAmounts[MAX_LIGHT_SET] = { 30000, 15000, 8000, 3000 };
...
CAnimateSaver::CAnimateSaver( PTSaverModeData ssData ) : CScreenSaver( ssData )
{
    ...
    flashColorIndex = 0;
    carTimer = 0;
    renderLightningBackwards = FALSE;
...
BOOL CAnimateSaver::LoadSound()
{
    ...
    if( SaverActive() && Mode() == ssSaver && Settings.GetSoundOn() )
    {
        ...
        if( ManySndDevices )
        {
            ...
            for( i = 0; i < MAX_REG_SOUNDS; ++i )
            {
                if( !sndMjr.ApplyLoadToDevice( i, i ) )
                    goto HANDLE_ERROR;
            }
            ...
            {
                sprintf( tmpStr, "soundThunder%d", i );
            ...
            if( !sndMjr.Play( SND_RAIN, 0xFFFFFFFF ) )
				goto HANDLE_ERROR;
        }
...
BOOL CAnimateSaver::InitSaverData()
{
    ...
    flashTimer = timeGetTime() + flashTimeOutAmounts[Settings.GetLightHits()] + (rand() % RAND_LIGHTNING_TIMEOUT);
    lightTimer = timeGetTime() + HIGH_DELAY;
    lightDurTimer = timeGetTime() + HIGH_DELAY;
...
void CAnimateSaver::UpdateAllAnimations()
{
    ...
    if( flashTimer < timer )
    {
        ...
        // Reset the timer so high that it doesn't come back rightaway
        flashTimer = timer + HIGH_DELAY;

        // Set the lightning timer
        lightTimer = timer + FLASH_TIMEOUT;
    ...
    else if( lightTimer < timer )
    {
        ...
        // Do we render the lightning forwards or backwards
        renderLightningBackwards = rand() % 2;

        // Reset the timer so high that it doesn't come back rightaway
        lightTimer = timer + HIGH_DELAY;
    ...
    else if( lightDurTimer < timer )
    {
        ...
        // Reset the timer so high that it doesn't come back rightaway
        lightDurTimer = timer + HIGH_DELAY;
        ...
        {
            // Reset the time to the next lightning flash
            flashTimer = timer + flashTimeOutAmounts[Settings.GetLightHits()] + (rand() % RAND_LIGHTNING_TIMEOUT);
...
void CAnimateSaver::AnimateRain()
{
    int i;
...
void CAnimateSaver::AnimateCity()
{
    int i;
...
void CAnimateSaver::AnimateLightning()
{
    RECT tmpRect;
    ...
    if( lightningActive && updateBuffer )
    {
        ...
        // Render the lightning backwards
        if( renderLightningBackwards )
        {
...
void CAnimateSaver::AnimateNeighborhood()
{
...
BOOL CAnimateSaver::LoadMetaFileAnim( PTMetafile pMetaFile, char *ResName, int noOfAnim )
{
    ...
    for( i = 0; i < noOfAnim; ++i )
    {
        ...
        // Init the metafile class with the active DC.
        // We already have it so it doen't need to be made again
        pMetaFile[i].InitActiveDC( screenDC );

        // Load the metafile
        if( !pMetaFile[i].LoadFromResource( hInst, metaResName, "WMF" ) )
        {
            Result = FALSE;
            break;
        }
...
void CAnimateSaver::ClearVideoBuffer( WORD color )
{
    int i, size;
    DWORD register *vidBuff;
    DWORD wordColor;

    // Memset faster for black color
    if( color == 0 )
    {
        // get the size in byte length
        size = (buffer.width() * buffer.height()) << 1;

        // We only need to update a 3/4th of the buffer
        size -= (size >> 2);

        memset( buffer.GetPtrToDIBsectionBits(), 0, size );
    }

    // Memset faster for white color
    else if( color == 0xFFFF )
    {
        ...
        memset( buffer.GetPtrToDIBsectionBits(), 0xFF, size );
    }
    else
    {
