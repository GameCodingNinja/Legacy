...
************************************************************************/
void CReel::InitSymbols( CDib * pSymb )
{
    int i;
    ...
    blitMaxY = 108;
    blitMaxH = (symbH * 3) + blitMaxY;
    offsetX = 75;
    offsetY = blitMaxY + symbH;

    for( i = 0; i < MAX_SYMBOL_CYCLE; ++i )
    {
        symb[i].index = i;
        symb[i].defPos = -(symbH * 2) + (i * symbH);
        symb[i].vector = 0;
    }
...
void CReel::Animate( int vector )
{
    ...
    for( i = 0; i < MAX_SYMBOL_CYCLE; ++i )
    {
        ...
        if( vector > 0 )
        {
            ...
            if( symb[i].vector >= symbH )
            {
                symb[i].vector = symb[i].vector - symbH;
                --symb[i].index;
        ...
        SetRect( &dst, offsetX, offsetY + symb[i].defPos + symb[i].vector, symbW, symbH );
        SetRect( &src, 0, offsetY + symb[i].defPos + symb[i].vector, symbW, symbH );
        ...
        if( ClipHeight( src ) )
        {
            ...
            //int symbIndex = pPhyReel->phyReel[ symb[i].index ];
			unsigned char *bits = pSymbol[pPhyReel->phyReel[ symb[i].index ]].bits();

			if( dst.top + symbH > blitMaxH )
				bits += ((symbH - src.bottom) * (symbW * 3));
			 
            glRasterPos2i( offsetX, dst.top + src.bottom );
			glDrawPixels( symbW, src.bottom, GL_RGB, GL_UNSIGNED_BYTE,
			              bits  );
        } 
    }
