...
bool CTextureLib::LoadFromFile( char *fileName, ERM_RENDER_DEVICE device )
{
    ...
    if( count > 0 )
    {
        ...
        else if( device == ERD_DIRECTX )
        {
            ...
            for( i = 0; i < count; ++i )
            {
                if( !pTMap[ i ].LoadFromLibraryFile_DirectX( hFile, autoMipmap ) )
                    goto HANDLE_ERROR;
            }
			...
			if( autoMipmap )
			{
				g_pDXDevice->SetSamplerState( 0, D3DSAMP_MIPFILTER, D3DTEXF_POINT );
				//g_pDXDevice->SetSamplerState( 0, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR );
			}
			#endif
        }
