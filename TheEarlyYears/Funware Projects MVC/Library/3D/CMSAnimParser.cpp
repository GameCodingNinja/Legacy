...
bool CMSBjParser::SaveToFile()
{
    ...
    RelpaceFileExtension( NULL, fileName, ".meshAnim" );
...
bool CMSBjParser::GenerateNormalFile()
{
	...
	tmpAnimMshLst.CalcVertexNormals();
...
bool CMSBjParser::BuildMapFile()
{
    ...
    for( int anim = 0; anim < animLst.Count(); ++anim )
    {
        ...
        for( int grp = 0; grp < animLst.Get(anim)->frameLst.Get(0)->groupLst.Count(); ++grp )
        {
    ...
    for( int anim = 0; anim < animLst.Count(); ++anim )
    {
        ...
        for( int frm = 0; frm < animLst.Get(anim)->frameLst.Count(); ++frm )
        {
            fprintf( hFile, "\n    Frame: %d\n", frm + 1 );

            // List all the vertices
            for( int vert = 0; vert < animLst.Get(anim)->frameLst.Get(frm)->gVertLst.Count(); ++vert )
            {
                // Get the vertex data
                pVert = animLst.Get(anim)->frameLst.Get(frm)->gVertLst.Get(vert);
            ...
            for( int grp = 0; grp < animLst.Get(anim)->frameLst.Get(frm)->groupLst.Count(); ++grp )
            {
            ...
            for( int grp = 0; grp < animLst.Get(anim)->frameLst.Get(frm)->groupLst.Count(); ++grp )
            {
                ...
                for( int fac = 0; fac < animLst.Get(anim)->frameLst.Get(frm)->groupLst.Get(grp)->faceList.Count(); ++fac )
                {
                    // Get a pointer to the face in the group
                    pFace = animLst.Get(anim)->frameLst.Get(frm)->groupLst.Get(grp)->faceList.Get(fac);
    ...
    for( int txt = 0; txt < textStrLst.Count(); ++txt )
    {
