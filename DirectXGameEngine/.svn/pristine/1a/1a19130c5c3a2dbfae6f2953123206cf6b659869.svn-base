...
{
    /*if( CStage::Instance().GetLightList().IsShadowCast() && (spGameState->GetState() == EGS_RUN) )
    {
        CMatrix testLightPos;

        //testLightPos.Translate( CStage::Instance().GetLightList().GetShadowCastLightPos() );
        //testLightPos.Rotate( CStage::Instance().GetLightList().GetShadowCastLightRot() );

        testLightPos.Translate( CPoint(0,300,0) );
        testLightPos.Rotate( CPoint(-90,0,0) );

        CStage::Instance().RenderShadowMap( testLightPos );
    }*/
