    ...
    for( uint i = 0; i < pScrollBoxCtrl->GetScrollCtrlCount(); ++i )
    {
        ...
        if( pCtrl1->GetState() != NUIControl::ECS_DISABLED )
        {
            for( uint dev = 0; dev < NDevice::MAX_UNIQUE_DEVICES; ++dev )
            {
                ...
                {
                    bool found(false);
                    string actionStr = pCtrl1->GetDisplayedString();
                    string componetStrId1 = pCtrl1->GetSubControl(dev)->GetDisplayedString();
                    size_t strPos = pCtrl1->GetDisplayedString().find("Menu");
                    bool menuItem1 = ((strPos != string::npos) && (strPos == 0));
                    ...
                    for( uint j = i+1; j < pScrollBoxCtrl->GetScrollCtrlCount(); ++j )
                    {
                        ...
                        {
                            string componetStrId2 = pCtrl2->GetSubControl(dev)->GetDisplayedString();
                            strPos = pCtrl2->GetDisplayedString().find("Menu");
                            bool menuItem2 = ((strPos != string::npos) && (strPos == 0));
