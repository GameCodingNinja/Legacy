...
bl_info = {
    "name": "Export 3D Mesh Format(.3dm)",
    "author": "Rosenorn",
    "version": (1, 0),
    "blender": (2, 5, 9),
    "api": 37702,
    "location": "File > Export > Rabid Squirrel Games (.3dm)",
...
import time
import io_export_rabidsquirrel_header as header
from ctypes import *
...
#                 exportAnimFrames=False):
#        self.exportCollisionMesh = exportCollisionMesh
#        self.exportAnimFrames = exportAnimFrames
...
def export_mesh(exportData):
    ...
    # Create the face groups sorted by first texture of face material
...
#************************************************************************
#    desc:  export mesh as a collision mesh
# 
#    param: exportData
#************************************************************************
def export_collision(exportData):
    ...
    for mesh in exportData.meshLst:
        for face in mesh.faces:
           AddFaceToGrp_collision(exportData.totalVertCount, face, exportData.faceLst)
...
#                 the hard drive
# 
#    param: exportData -  export data class holding all data to help
#                         keep things simple
#************************************************************************
...
#                         keep things simple
# 
#    NOTE: The above dictionaries are used to keep duplicate data out.
#************************************************************************
...
#************************************************************************
#    desc:  Create face group list for collision meshes
# 
#    param: meshVertOffset - offset to add to the vert index
#           face the face to add
#           faceLst - face list to add it to
#************************************************************************
def AddFaceToGrp_collision(meshVertOffset, face, faceLst):
...
def TextWriteFaceIndexes(exportData):
    if exportData.faceGrpLst:
        ...
        for faceGrp in exportData.faceGrpLst:
            ...
            # NOTE: The order of the verts has been changed from 0,1,2 to 0,2,1 to convert to left hand system
            for face in faceGrp.faceLst:
                exportData.txtFile.write("  {}, {}, {}".format(face.vertIndexLst[0], face.vertIndexLst[2], face.vertIndexLst[1]))
                exportData.txtFile.write("    {}, {}, {}".format(face.uvIndexLst[0], face.uvIndexLst[2], face.uvIndexLst[1]))
                exportData.txtFile.write("    {}, {}, {}\n".format(face.normIndexLst[0], face.normIndexLst[2], face.normIndexLst[1]))
        ...
        # NOTE: The order of the verts has been changed from 0,1,2 to 0,2,1 to convert to left hand system
        for face in exportData.faceLst:
            exportData.txtFile.write("  {}, {}, {}\n".format(face.vertIndexLst[0], face.vertIndexLst[2], face.vertIndexLst[1]))
...
def TextWriteTextures(exportData):
    ...
    for textureSlot in exportData.uniqueTextureLst:
        ...
        exportData.txtFile.write(textureType + textureSlot.texture.image.filepath.lstrip("/.\\") + "\n")
...
def BinaryWriteKeyFrameData(exportData):
    if exportData.armatureObj is not None:
        ...
        # write the joint tag check
        tagCheck.tagCheck = header.JOINT;
        ...
        for keyFrameGrp in exportData.keyFrameGrpLst:
            ...
            # write the key frame tag check
            tagCheck.tagCheck = header.KEY_FRAME;
            exportData.binaryFile.write(tagCheck)
    ...
    # Export the vert data without bones
        ...
        for mesh in exportData.meshLst:
            for vert in mesh.vertices:
                ...
                binaryVert.y = vert.co.z
                binaryVert.z = vert.co.y
...
def BinaryWriteFaceIndexes(exportData):
    ...
    if exportData.faceGrpLst:
        ...
        for faceGrp in exportData.faceGrpLst:
            ...
            # NOTE: The order of the verts has been changed from 0,1,2 to 0,2,1 to convert to left hand system
            binaryFace = header.CBinaryFace()
            for face in faceGrp.faceLst:
                binaryFace.vert_1 = face.vertIndexLst[0]
                binaryFace.vert_2 = face.vertIndexLst[2]
                binaryFace.vert_3 = face.vertIndexLst[1]
                binaryFace.uv_1 = face.uvIndexLst[0]
                binaryFace.uv_2 = face.uvIndexLst[2]
                binaryFace.uv_3 = face.uvIndexLst[1]
                binaryFace.norm_1 = face.normIndexLst[0]
                binaryFace.norm_2 = face.normIndexLst[2]
                binaryFace.norm_3 = face.normIndexLst[1]
                exportData.binaryFile.write(binaryFace)
    else:
        # write the face group tag check
        tagCheck.tagCheck = header.FACE_LIST;
        exportData.binaryFile.write(tagCheck)

        # write the face list info
        faceInfo = header.CFaceCount()
        faceInfo.faceCount = len(exportData.faceLst)
        exportData.binaryFile.write(faceInfo)
    
        # NOTE: The order of the verts has been changed from 0,1,2 to 0,2,1 to convert to left hand system
        binaryFace = header.CBinaryVertFace()
        for face in exportData.faceLst:
            binaryFace.vert_1 = face.vertIndexLst[0]
            binaryFace.vert_2 = face.vertIndexLst[2]
            binaryFace.vert_3 = face.vertIndexLst[1]
...
def BinaryWriteTextures(exportData):
    ...
    for textureSlot in exportData.uniqueTextureLst:
        ...
        texturePath.path = textureSlot.texture.image.filepath.lstrip("/.\\").encode('utf-8')
...
def BinaryWriteBones(exportData):
    ...
    # write the bone tag check
    tagCheck.tagCheck = header.JOINT_LIST;
    exportData.binaryFile.write(tagCheck)
    ...
    for bone in exportData.dataBoneLst:
        ...
        binaryJoint.headY = headBoneVec.z
        binaryJoint.headZ = headBoneVec.y
        ...
        binaryJoint.tailY = tailBoneVec.z
        binaryJoint.tailZ = tailBoneVec.y
...
#************************************************************************
class Export_RabidSquirrel(bpy.types.Operator):
    '''Exports scene as 3D Mesh Format .3dm file.'''
    ...
    exportArmature = BoolProperty(name="Export Armature (when available)", description="Export the mesh and armature if the armature is present", default=True)
...
def menu_func(self, context):
    self.layout.operator(Export_RabidSquirrel.bl_idname, text="Export Rabid Squirrel (.3dm)")
