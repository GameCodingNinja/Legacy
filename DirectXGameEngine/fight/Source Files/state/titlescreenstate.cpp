...
bool CTitleScreenState::DoStateChange()
{
    if( CUIMenuTree::Instance().GetState() == NUIMenu::EMTS_HIDDEN )
    {
        ...
        // Set the physics worlds to activate
        stateMessage.startPhysicsWorld.push_back("game");
        ...
        stateMessage.createMegaTexture.push_back( boost::tuple<std::string, int>("(actors)",512) );
