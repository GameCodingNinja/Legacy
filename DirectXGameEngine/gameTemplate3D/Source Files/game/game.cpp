...
#include "../smartGUI/smartanisotropicbtn.h"
...
#include "../smartGUI/smartscrollbox.h"
...
{
    if( pEffectData->effectNameStr == "gausian_lights" )
    {
        CShader::Instance().SetEffectValue( pEffectData, "enableLights", true );
        CShader::Instance().SetEffectValue( pEffectData, "shadowMapBufferTexture", CXDevice::Instance().GetShadowMapBufferTexture() );
        CShader::Instance().SetEffectValue( pEffectData, "textureFilter", CXDevice::Instance().GetTextureFiltering( CSettings::Instance().GetTextFiltering() ) );
        CShader::Instance().SetEffectValue( pEffectData, "anisotropyRate", CXDevice::Instance().GetAnisotropicFiltering( CSettings::Instance().GetTextFiltering() ) );

        CStage3D::Instance().GetLightList().SetShaderLights( pEffectData );
    }
    else if( pEffectData->effectNameStr == "post_process" )
    {
...
void CGame::PerformSmartGuiCreate( CUIControl * pUIControl, CUIMenu * pUIMenu )
{
    ...
    if( pUIControl != NULL )
    {
        ...
        else if( pUIControl->GetName() == "anisotropic_btn_lst" )
            pUIControl->SetSmartGui( new CSmartAnisotropicBtn( pUIControl ) );
...
{
    /*if( CStage3D::Instance().GetLightList().IsShadowCast() && (spGameState->GetState() == EGS_RUN) )
    {
        CMatrix testLightPos;

        //testLightPos.Translate( CStage3D::Instance().GetLightList().GetShadowCastLightPos() );
        //testLightPos.Rotate( CStage3D::Instance().GetLightList().GetShadowCastLightRot() );

        testLightPos.Translate( CPoint(0,300,0) );
        testLightPos.Rotate( CPoint(-90,0,0) );

        CStage3D::Instance().RenderShadowMap( testLightPos );
    }*/
...
****************************************************************************/
bool CGame::SetConvertPixelsToTexels( string & effectId, string & convertPixelsId )
{
